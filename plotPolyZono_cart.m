...
y_labels = {'$x$', '$\theta$'};
% Plot rectangles sets
for dim=1:2
    figure('Renderer', 'painters', 'Position', [10 10 700 900]);
    hold on;
    box on;
    for i=1:maxsteps-1
        cur_int = Rplotall{i}(dim);
        handR=  plot([t_vec(i), t_vec(i)], [cur_int.inf, cur_int.sup],'r');
        % plot(YPred(projectedDims{plotRun}(1),i),YPred(projectedDims{plotRun}(2),i),'*-k');
    end
    for i=1:maxsteps
        haldy_t= plot(t_vec(i),y_t(dim,i),'+b');
        %         else
        haldy_Pred= plot(t_vec(i),YPred(dim,i),'*k');
        %         end
    end
    % handC=plot(consSet,projectedDims{plotRun},'k');
    legend([handR,haldy_t,haldy_Pred],'Reachable set $\hat{\mathcal{R}}_t$','System trajectory $y(t)$','$y$-pred$(t)$','Interpreter','latex')
    ...
    ylabel(y_labels{dim},'Interpreter','latex');
    grid on
...
han_uref=plot(uref.*ones(1, size(uPred,2)),'k-');
ax = gca;
xlabel('Simulation time, $t$ (sec)','Interpreter','latex')
...
box on;
%for i =1:dim_x
handy{1}= plot(t_vec, y_t(1,:)','r');
handy_pred{1}= plot(t_vec, YPred(1,:)','-*r');
handy{2}= plot(t_vec, y_t(2,:)','k');
handy_pred{2}= plot(t_vec, YPred(2,:)','-*k');
handy{3}= plot(t_vec, y_t(3,:)','b');
handy_pred{3}= plot(t_vec, YPred(3,:)','-*b');
handy{4}= plot(t_vec, y_t(4,:)','g');
handy_pred{4}= plot(t_vec, YPred(4,:)','-*g');
%end
...
xlim([0,Tf])
xlabel('Simulation time, $t$ (sec)','Interpreter','latex')
legend([handy{1},handy_pred{1},handy{2},handy_pred{2},handy{3},handy_pred{3},handy{4},handy_pred{4}],...
    '$y_1(t)$','$y_1$-pred$(t)$','$y_2(t)$','$y_2$-pred$(t)$','$y_3(t)$','$y_3$-pred$(t)$','$y_4(t)$','$y_4$-pred$(t)$','Interpreter','latex');
ax = gca;
ax.FontSize = 19;
%set(gcf, 'Position',  [50, 50, 800, 400])
ax = gca;
...
hold on
box on;
han_yt2ref_poly=plot(t_vec(1:end-1), yt2ref_poly,'r-');
...
hold on
box on;
hand_y_t= plot(t_vec, y_t(1,:),'b*-');
hand_y_poly= plot(t_vec, Y(1,:),'r-');
hand_y_t_model= plot(t_vec, y_t_model(1,:),'r+-');
hand_YPred_t=plot(t_vec, YPred(1,:),'b+-');
...
hold on
box on;
hand_y_t= plot(t_vec, y_t(2,:),'b*-');
hand_y_poly= plot(t_vec, Y(2,:),'r-');
hand_y_t_model= plot(t_vec, y_t_model(2,:),'r+-');
hand_YPred_t=plot(t_vec, YPred(2,:),'b+-');

handcon = plot(t_vec, intc.inf(2)*ones(size(y_t(2,:))),'k--');
handcon = plot(t_vec, intc.sup(2)*ones(size(y_t(2,:))),'k--');
