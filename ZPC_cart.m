...
% System in cont time
A = [0, 0, 1, 0; 0, 0, 0, 1; 0, mp*g/mc, 0, 0; 0, (mp+mc)*g/(mc*l), 0, 1];
...
figure;
subplot(2,1,1)
hold on; box on; plot(t_vec, x(3,:),'b'); xlabel('t'); ylabel('\dot{x}'); axis equal;
subplot(2,1,2)
hold on; box on; plot(t_vec, x(4,:),'b'); xlabel('t'); ylabel('\dot{\theta}'); axis equal;
close all;
disp(max(x(2:4:end)))
